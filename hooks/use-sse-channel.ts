...
interface UseSSEChannelOptions {
  onNewMessages?: (data: SSEChannelData) => void;
  autoRefresh?: boolean;
  refreshDelay?: number;
}
...
export function useSSEChannel(
  ...
  const updatePollingInterval = useCallback(() => {
    ...
    if (newInterval !== pollingInterval) {
      ...
      console.log(`[SSE_CHANNEL] Polling interval updated to ${newInterval}ms`);
    }
  }, [pollingInterval]);
  ...
  const connect = useCallback(() => {
    ...
    const connectionId = `${channelId}-${Date.now()}`;
    ...
    try {
      ...
      eventSource.onopen = () => {
        ...
        if (connectionIdRef.current === connectionId) {
          ...
          console.log(`[SSE_CHANNEL] Connected to channel: ${channelId} (${connectionId})`);
        }
      };
      ...
      eventSource.onmessage = (event) => {
        ...
        try {
          const data: SSEChannelData = JSON.parse(event.data);
          
          switch (data.type) {
            case 'connected':
              console.log(`[SSE_CHANNEL] Initial connection confirmed for channel: ${channelId}`);
              break;
            ...
            case 'new_messages':
              ...
              console.log(`[SSE_CHANNEL] New messages received:`, {
            ...
            case 'error':
              ...
              console.error(`[SSE_CHANNEL] Server error:`, data.message);
              break;
              
            default:
              console.log(`[SSE_CHANNEL] Unknown event type:`, data.type);
          }
        } catch (err) {
          console.error('[SSE_CHANNEL] Failed to parse message:', err);
      ...
      eventSource.onerror = (event) => {
        ...
        console.error(`[SSE_CHANNEL] Connection error for channel: ${channelId}`, event);
      ...
      console.error('[SSE_CHANNEL] Failed to create EventSource:', err);
      setConnectionState('error');
      setError('Failed to establish connection');
    }
  }, [channelId, onNewMessages, autoRefresh, refreshDelay]);
  ...
  }, [channelId]); // Only depend on channelId, not connect/disconnect
